...
// Los mensajes de bienvenida, fallback y escalado se obtienen
// de la config remota (panel admin) con fallback a defaults.
// ══════════════════════════════════════════════════════════════

@Injectable()
export class MessageFormatterService {

  constructor(private readonly remoteConfig: BotRemoteConfigService) {}

  // ─── Bienvenida ───────────────────────────────────────────
  welcome(name: string): string {
    const cfg = this.remoteConfig.get();
    const greeting = cfg.Mensajes.Bienvenida.replace('{{nombre}}', name).replace('{{empresa}}', 'Telecom Bolivianet');
    const menuText = this.remoteConfig.buildMenuText();
    return (
      `${greeting}\n\n` +
      `${cfg.Menu.TituloMenu}\n\n` +
      `${menuText}\n\n` +
  ...
  // ── US-EP03-01: Respuesta RAG con referencia a la fuente ─────────────────
  //
  // Muestra al usuario de qué documento proviene la respuesta.
  // Aumenta la confianza: el cliente sabe que es información oficial
  // de Bolivianet y no una invención del LLM.
  //
  // Formato:
  ...
  //   ¿Se resolvió? Sí / No
  //   ─────────────────
  //   📄 Fuente: Manual técnico Bolivianet 2024
  ragSupportGuide(answer: string, documentTitle?: string): string {
    const sourceFooter = documentTitle
  ...
  // ── US-EP03-01: Respuesta RAG general (para handleRag del orquestador) ────
  //
  // Para respuestas RAG que no son de soporte técnico (preguntas generales).
  // Incluye la fuente del documento al final.
  ragAnswer(answer: string, documentTitle?: string): string {
    const sourceFooter = documentTitle
      ? `\n\n─────────────────\n📄 *Fuente:* ${documentTitle}`
      : '';

    return `${answer}${sourceFooter}`;
  }
  ...
  escalationNotice(): string {
    return this.remoteConfig.get().Mensajes.EscaladoAgente;
  }
